...
//!
//!   * `Invariant` / `InvariantChecker` — runs a set of invariant predicates
//!     against a synthesized `SystemState` for property tests.
//!   * The concrete invariants (`BalanceConservationInvariant`,
//!     `TransactionIntegrityInvariant`, `SessionValidityInvariant`).
//!   * `strategies` — proptest `Strategy` generators for synthetic state.
//!   * `PropertyTestRunner` / `TestResults` — drives the invariant suite.
//!   * `formal` — placeholder for formal-verification metadata.
//!
//! At the time of split, zero call sites outside this module imported any of
//! these types. They are still `pub` so future proptest harnesses can pull
//! them in via `crate::verification::formal_infra::*`.
//!
