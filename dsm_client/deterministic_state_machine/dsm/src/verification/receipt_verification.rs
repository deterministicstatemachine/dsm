...
pub fn verify_stitched_receipt(
    ...
    // Rule 2b: device-birth-bound ephemeral-key authorization (whitepaper §11.1).
    ...
    let Some(chain_head) = &ctx.chain_head_pubkey_a else {
        return Ok(ReceiptAcceptance::reject(
            "Missing chain_head_pubkey_a (device-birth EK cert chain required)".to_string(),
        ));
    };
    if receipt.ek_cert_a.is_empty() {
        return Ok(ReceiptAcceptance::reject(
            "Missing ek_cert_a (device-birth EK cert chain required)".to_string(),
        ));
    }
    ...
    let Some(chain_head) = &ctx.chain_head_pubkey_b else {
        return Ok(ReceiptAcceptance::reject(
            "Missing chain_head_pubkey_b (device-birth EK cert chain required)".to_string(),
        ));
    };
    if receipt.ek_cert_b.is_empty() {
        return Ok(ReceiptAcceptance::reject(
            "Missing ek_cert_b (device-birth EK cert chain required)".to_string(),
        ));
    }
    match crate::crypto::ephemeral_key::verify_ek_cert(
        chain_head,
...
mod tests {
    ...
    /// also verify the device-birth-bound per-step EK cert chain.
    #[test]
    fn test_no_chain_head_rejects_receipt_authorization() {
        ...
        let reason = result.reason.unwrap();
        assert!(
            reason.contains("chain_head_pubkey_a") || reason.contains("device-birth"),
            "wrong rejection reason: {}",
            reason
        );
    }
