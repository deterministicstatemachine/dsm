...
//!
//! Per §4.3, no counter/height/timestamp participates in state identity or
...
impl StateBuilder {
    ...
    pub fn build(mut self) -> Result<State, DsmError> {
        // Calculate state hash if not set. Per §4.3 no counter participates.
        if self.hash == [0u8; 32] {
        ...
        let params = crate::types::state_types::StateParams::new(entropy, operation, device_info)
...
mod tests {
    ...
    fn build_minimal_valid_state() {
        let state = valid_builder().build().expect("builds");
    ...
    fn build_with_explicit_hash() {
        let hash = [0xFF; 32];
        let state = valid_builder().with_hash(hash).build().expect("builds");
        assert_eq!(state.hash, hash);
    }

    #[test]
    fn build_auto_computed_hash_is_deterministic() {
        let h1 = valid_builder().build().expect("builds").hash;
        let h2 = valid_builder().build().expect("builds").hash;
        assert_eq!(h1, h2);
    }

    #[test]
    fn build_different_entropy_produces_different_hashes() {
        let h1 = valid_builder()
            .with_entropy(vec![0x01; 32])
            .build()
            .expect("builds")
            .hash;
        let h2 = valid_builder()
            .with_entropy(vec![0x02; 32])
            .build()
            .expect("builds")
            .hash;
        assert_ne!(h1, h2);
    ...
    fn build_with_token_balance() {
        let balance = Balance::from_state(500, [0; 32]);
        let state = valid_builder()
    ...
    fn build_with_token_balances_map() {
        ...
        balances.insert("ERA".into(), Balance::from_state(100, [0; 32]));
        balances.insert("TEST".into(), Balance::from_state(200, [0; 32]));
        let state = valid_builder()
    ...
    fn build_relationship_context_auto() {
        let state = valid_builder()
            ...
            .build()
            .expect("builds");
        let ctx = state.relationship_context.expect("has context");
