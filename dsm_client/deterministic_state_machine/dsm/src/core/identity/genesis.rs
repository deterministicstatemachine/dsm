...
fn calculate_initial_entropy(
    ...
    let mut hasher = dsm_domain_hasher(crate::crypto::domain::TaggedHashDomain::from_static(
        b"DSM/genesis-initial-entropy",
    ));
...
fn calculate_device_entropy(
    ...
    let mut hasher = dsm_domain_hasher(crate::crypto::domain::TaggedHashDomain::from_static(
        b"DSM/sub-genesis-device-entropy",
    ));
...
pub fn create_genesis_v3_self_attested(
    ...
    let state = GenesisState {
        hash: v3.g,
        initial_entropy: v3.genesis_nonce, // public, deterministic; no MPC entropy
        participants: HashSet::new(),      // no storage nodes for the mnemonic profile
        merkle_root: None,
        device_id: Some(v3.devid),
...
mod tests {
    ...
    fn create_genesis_v3_self_attested_is_deterministic_and_recoverable() {
        ...
        assert_eq!(a.state.device_id, Some(v3.devid));
