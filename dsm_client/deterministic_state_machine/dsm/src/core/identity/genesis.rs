...
//! - device-birth is a local, optional anti-cloning signal; it must not be required for
...
// -------------------- Device entropy (no device-birth) --------------------
...
pub fn convert_session_to_genesis_state_compat(
    ...
    // — exactly the bytes the session hashed into `G`. Metadata and device-birth are
