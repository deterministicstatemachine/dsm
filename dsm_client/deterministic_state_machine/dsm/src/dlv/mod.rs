...
//! Tier 2 Foundation DLV primitives â pure-crypto helpers that the
...
pub mod beta_storage_profile; // the deployed three-member beta profile â fixed, not a formula
pub mod controller_rotation;
pub mod pair_identity;
pub mod quorum_bind; // Def 6.21 â Class K sans-IO quorum-binding decision engine
...
pub mod trader_fence; // Req 6.23 — the initiating-trader parent fence (pure state machine)
...
pub mod vault_state_anchor_v3; // Def 6.4a â owner baseline over c_n; the only anchor form after the cut
