...
fn faucet_claim_cannot_install_balance_without_the_admission_fence() {
    ...
    let other = claim_op(era_reserve_id(NETWORK), 43);
    let err = fenced
        .advance(
            rel,
            devid,
    ...
    // 4. Generation 0 is the genesis state: refused before anything else.
    let g0 = claim_op(era_reserve_id(NETWORK), 0);
    assert!(head
        .advance(rel, devid, g0, &[delta], Some(tip), None, None,)
        .is_err());
}
