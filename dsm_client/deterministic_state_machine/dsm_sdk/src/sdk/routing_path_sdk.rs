...
/// Tier 2 N-best path search.  Returns up to `k` distinct paths from
/// `input_token` to `output_token`, ordered by final output amount
/// (descending) with the same deterministic vault-id tie-break used in
/// `find_best_path`.
///
/// Used by the RouteCommit binder to construct envelope fallbacks: if
/// the primary path's vault state moves between quote and unlock, the
/// wallet attempts each subsequent path in order under the same signed
/// envelope.  No re-signing required because all paths share the
/// envelope's nonce + initiator + intent bounds.
///
/// Returns at least one path on success (the best); fewer than `k` if
/// the graph topology doesn't support that many distinct paths.
pub(crate) fn find_n_best_paths(
    advertisements: &[generated::RoutingVaultAdvertisementV1],
    input_token: &[u8],
    output_token: &[u8],
    input_amount: u128,
    ...
    // Sort by (output desc, vault-id seq asc) for deterministic ordering.
    all.sort_by(
        |a, b| match b.final_output_amount.cmp(&a.final_output_amount) {
            std::cmp::Ordering::Equal => {
                let a_seq: Vec<&[u8]> = a.hops.iter().map(|h| h.vault_id.as_slice()).collect();
                let b_seq: Vec<&[u8]> = b.hops.iter().map(|h| h.vault_id.as_slice()).collect();
                a_seq.cmp(&b_seq)
            }
            other => other,
        },
    ...
    // Drop exact duplicates (same vault-id sequence). Distinct paths
    // must differ in at least one vault id; otherwise they're the same
    // path computed via different enumeration orders.
    let mut seen: Vec<Vec<[u8; 32]>> = Vec::new();
    all.retain(|p| {
        let seq: Vec<[u8; 32]> = p.hops.iter().map(|h| h.vault_id).collect();
...
/// Tier 2 verified N-best wrapper.  Same digest-verification semantics
/// as `find_and_verify_best_path` (tampered ads are silently dropped
/// before search) but returns up to `k` distinct candidate paths so a
/// caller can bind them into one signed envelope with fallback groups.
///
/// Returns at least one path on success; fewer than `k` if the graph
/// topology doesn't support that many distinct paths.
pub(crate) async fn find_and_verify_n_best_paths(
    advertisements: &[generated::RoutingVaultAdvertisementV1],
    input_token: &[u8],
    output_token: &[u8],
    input_amount: u128,
