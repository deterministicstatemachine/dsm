...
// —————————————————————————–
// Platform device-birth input (the SDK-owned birth-nonce commitment)
//
// Transient, process-lifetime-only hand-off slot for the single 32-byte
// device-birth nonce commitment.  rules.instructions.md makes Rust the sole
// crypto authority, so the SDK (not the host) generates the commitment at the
// top of `system.genesis` and stages it here; this slot only carries it across
// the MPC genesis call.  The host supplies no binding material.
//
...
/// The SDK-owned device-birth nonce commitment, held in a transient
/// process-lifetime slot for consumption by the genesis create flow.
///
/// Lifecycle:
///   1. The `system.genesis` handler generates the commitment and stages it via
///      `AppState::set_device_birth_nonce_commitment(commitment)`.
///   2. The genesis create flow calls
///      `AppState::peek_device_birth_nonce_commitment()` mid-flow so it can
///      fold the commitment into `AttA` (the slot must remain readable across
///      the MPC call).
///   3. After `AttA` has been installed via `binding_key::install_binding_key`,
///      the caller invokes `AppState::take_device_birth_nonce_commitment()` to
///      consume + zeroize the slot.
#[derive(Debug, Clone, Zeroize, ZeroizeOnDrop)]
pub struct PlatformDeviceBirthInput {
    /// Commitment to a device-local 32-byte birth nonce
    /// (`BLAKE3("DSM/device-birth-nonce/v1\0" || birth_nonce)`), generated by
    /// the SDK at genesis.  Folded VERBATIM into `AttA` and persisted in the
    /// `GenesisRecord` so finalize/restore re-derive the same AttA.  Must be 32
    /// bytes; strict mode rejects a wrong length.
    pub device_birth_nonce_commitment: Vec<u8>,
}

static PLATFORM_DEVICE_BIRTH_INPUT: Mutex<Option<PlatformDeviceBirthInput>> = Mutex::new(None);
...
impl AppState {
    ...
    // ----------------- Platform device-birth input (birth-nonce commitment) -----------------

    /// Store the host-supplied device-birth nonce commitment for consumption
    /// by the genesis create flow. Idempotent: a second call overwrites the
    /// prior value (which is zeroized on drop).
    ///
    /// The commitment MUST be 32 bytes. A wrong length is rejected here so the
    /// gate fires at the JNI boundary rather than deep inside the genesis path.
    ///
    /// Returns `Ok(())` on success or a structured error on a bad length.
    pub fn set_device_birth_nonce_commitment(
        device_birth_nonce_commitment: Vec<u8>,
    ) -> Result<(), String> {
        if device_birth_nonce_commitment.len() != 32 {
            return Err(format!(
                "device-birth: nonce commitment must be 32 bytes, got {}",
                device_birth_nonce_commitment.len()
            ));
        }

        let input = PlatformDeviceBirthInput {
            device_birth_nonce_commitment,
        };

        let mut slot = PLATFORM_DEVICE_BIRTH_INPUT
            .lock()
            .map_err(|e| format!("device-birth: lock poisoned: {e}"))?;
        *slot = Some(input);
        Ok(())
    }

    /// Non-consuming peek of the device-birth nonce commitment. Returns a
    /// `Clone` so the genesis create flow can fold it into `AttA` mid-flow.
    /// The slot stays populated until an explicit
    /// `take_device_birth_nonce_commitment` call zeroizes it.
    pub fn peek_device_birth_nonce_commitment() -> Option<PlatformDeviceBirthInput> {
        match PLATFORM_DEVICE_BIRTH_INPUT.lock() {
            Ok(slot) => slot.clone(),
            Err(poisoned) => poisoned.into_inner().clone(),
        }
    }

    /// One-shot consume the device-birth nonce commitment. Returns
    /// `Some(PlatformDeviceBirthInput)` exactly once after a successful
    /// `set_device_birth_nonce_commitment` call; subsequent calls return
    /// `None` until the platform layer sets a fresh commitment.
    ///
    /// The returned struct zeroizes its bytes on drop, so callers MUST drop
    /// it within a bounded scope after folding the commitment into `AttA`.
    pub fn take_device_birth_nonce_commitment() -> Option<PlatformDeviceBirthInput> {
        match PLATFORM_DEVICE_BIRTH_INPUT.lock() {
    ...
    /// Non-consuming check for whether a device-birth nonce commitment is
    /// currently set. Used by UI/JNI bootstrap readiness checks without
    /// consuming the slot.
    pub fn has_device_birth_nonce_commitment() -> bool {
        match PLATFORM_DEVICE_BIRTH_INPUT.lock() {
    ...
    pub fn reset_for_testing() {
        ...
        if let Ok(mut slot) = PLATFORM_DEVICE_BIRTH_INPUT.try_lock() {
            *slot = None;
        }
...
mod tests {
    ...
    #[test]
    #[serial]
    fn device_birth_commitment_initial_state_is_none() {
        setup_test_env();
        assert!(!AppState::has_device_birth_nonce_commitment());
        assert!(AppState::take_device_birth_nonce_commitment().is_none());
    }

    #[test]
    #[serial]
    fn device_birth_commitment_set_then_take_round_trip() {
        setup_test_env();
        let commitment = vec![0xAAu8; 32];

        AppState::set_device_birth_nonce_commitment(commitment.clone())
            .expect("set must succeed for a 32-byte commitment");

        assert!(AppState::has_device_birth_nonce_commitment());

        let taken = AppState::take_device_birth_nonce_commitment()
            .expect("take must return the commitment we just set");
        assert_eq!(taken.device_birth_nonce_commitment, commitment);
    ...
    #[test]
    #[serial]
    fn device_birth_commitment_rejects_wrong_length() {
        setup_test_env();
        let err = AppState::set_device_birth_nonce_commitment(vec![0xBB; 8])
            .expect_err("a non-32-byte commitment must be rejected");
        assert!(
            err.contains("32 bytes"),
            "error must name the length contract: {err}"
        );
        assert!(!AppState::has_device_birth_nonce_commitment());
    }
    ...
    #[test]
    #[serial]
    fn device_birth_commitment_set_overwrites_prior() {
        setup_test_env();

        AppState::set_device_birth_nonce_commitment(vec![0x01; 32]).expect("first set");
        AppState::set_device_birth_nonce_commitment(vec![0x10; 32])
            .expect("second set must overwrite, not error");

        let taken = AppState::take_device_birth_nonce_commitment()
            .expect("take should return the second-set value");
        assert_eq!(taken.device_birth_nonce_commitment, vec![0x10; 32]);
    }

    #[test]
    #[serial]
    fn device_birth_commitment_drives_att() {
        setup_test_env();
        // The whole point of the slot: folding the commitment from the slot
        // into DeviceBirthInputs::from_platform_nonce must produce the same
        // AttA as a direct call with the same 32 bytes.
        let commitment = b"device_birth_commitment_32_bytes".to_vec();

        AppState::set_device_birth_nonce_commitment(commitment.clone()).expect("set");
        let taken = AppState::take_device_birth_nonce_commitment().expect("take");

        use dsm::crypto::device_birth::{CreationMode, DeviceBirthInputs};
        let att_via_slot = DeviceBirthInputs::from_platform_nonce(
            &taken.device_birth_nonce_commitment,
            CreationMode::Genesis,
        )
        .derive_att();
        let att_direct =
            DeviceBirthInputs::from_platform_nonce(&commitment, CreationMode::Genesis).derive_att();
    ...
    fn peek_returns_clone_and_does_not_consume() {
        setup_test_env();
        let commitment = vec![0x33u8; 32];
        AppState::set_device_birth_nonce_commitment(commitment.clone()).expect("set");

        // Peek must return a clone of the slot.
        let peeked =
            AppState::peek_device_birth_nonce_commitment().expect("peek must see the set value");
        assert_eq!(peeked.device_birth_nonce_commitment, commitment);

        // Slot is still populated for a subsequent take.
        let taken =
            AppState::take_device_birth_nonce_commitment().expect("take must still see the value");
        assert_eq!(taken.device_birth_nonce_commitment, commitment);

        // After take, peek returns None.
        assert!(AppState::peek_device_birth_nonce_commitment().is_none());
    }
}
