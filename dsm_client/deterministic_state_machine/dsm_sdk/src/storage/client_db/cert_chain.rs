...
// specific. We persist it AEAD-encrypted under a key derived from AttA so:
//
// 1. Extracted ciphertext is useless on a different device — AttA binds
...
/// `AttA`. Whitepaper §12 binds this key to hardware/environment.
fn derive_chain_sk_aead_key(device_birth_att: &[u8; 32]) -> [u8; 32] {
...
pub fn encrypt_chain_sk(plain_sk: &[u8], device_birth_att: &[u8; 32]) -> Result<Vec<u8>> {
    let key = derive_chain_sk_aead_key(device_birth_att);
...
/// is tampered or if `device_birth_att` doesn't match what was used at encryption.
pub fn decrypt_chain_sk(ciphertext: &[u8], device_birth_att: &[u8; 32]) -> Result<Vec<u8>> {
    ...
    cipher
        ...
        .map_err(|_| anyhow!("cert-chain SK decryption failed (tamper or wrong AttA)"))
}
...
pub fn init_local_cert_chain_head_with_sk(
    ...
    let encrypted_sk = encrypt_chain_sk(chain_head_secret_key, device_birth_att)?;
    let binding = get_connection()?;
    let conn = binding.lock().unwrap_or_else(|p| p.into_inner());
    let now = tick() as i64;
    let inserted = conn.execute(
...
/// receipt is accepted. Encrypts the new SK under `AttA`. Returns the
...
pub fn advance_local_cert_chain_head_with_sk(
    ...
    let encrypted_sk = encrypt_chain_sk(new_secret_key, device_birth_att)?;
    let binding = get_connection()?;
    let conn = binding.lock().unwrap_or_else(|p| p.into_inner());
    let now = tick() as i64;
    let updated = conn.execute(
...
pub fn load_local_chain_head_sk(
    ...
    match ct {
        Some(ciphertext) if !ciphertext.is_empty() => {
            decrypt_chain_sk(&ciphertext, device_birth_att).map(Some)
        }
        _ => Ok(None),
    }
}
...
mod tests {
    ...
    fn chain_sk_aead_round_trip() {
        ...
        let device_birth_att = [0xCD; 32];
        let ct = encrypt_chain_sk(&plain, &device_birth_att).unwrap();
        // Nonce(24) + ciphertext(>=plain.len()) + tag(16) = at least 40 + plain.len()
        assert!(ct.len() >= 24 + plain.len() + 16);
        let recovered = decrypt_chain_sk(&ct, &device_birth_att).unwrap();
    ...
    fn chain_sk_aead_random_nonce_per_encryption() {
        ...
        let device_birth_att = [0x22; 32];
        let ct1 = encrypt_chain_sk(&plain, &device_birth_att).unwrap();
        let ct2 = encrypt_chain_sk(&plain, &device_birth_att).unwrap();
    ...
    fn chain_sk_aead_tamper_fails() {
        ...
        let mut ct = encrypt_chain_sk(&plain, &device_birth_att).unwrap();
        // Flip a bit in the ciphertext payload (after the 24-byte nonce).
        ct[30] ^= 0x01;
        assert!(decrypt_chain_sk(&ct, &device_birth_att).is_err());
    }

    /// Decrypting with a different AttA (different device) fails.
    #[test]
    #[serial_test::serial]
    fn chain_sk_aead_wrong_device_birth_att_fails() {
        let plain = vec![0x55u8; 64];
        let device_birth_att_a = [0x77; 32];
        let device_birth_att_b = [0x88; 32];
        let ct = encrypt_chain_sk(&plain, &device_birth_att_a).unwrap();
        assert!(decrypt_chain_sk(&ct, &device_birth_att_b).is_err());
    }

    /// Init-with-SK round-trip: encrypted SK survives the storage layer
    /// and decrypts cleanly under the right AttA.
    #[test]
    #[serial_test::serial]
    fn local_chain_head_sk_init_load_round_trip() {
        ...
        let inserted = init_local_cert_chain_head_with_sk(&r, &pk, &sk, &device_birth_att).unwrap();
        assert!(inserted);

        let loaded = load_local_chain_head_sk(&r, &device_birth_att).unwrap();
        assert_eq!(loaded, Some(sk.clone()));

        // Decrypting with wrong AttA fails.
        let bad = load_local_chain_head_sk(&r, &[0xDD; 32]);
        assert!(bad.is_err(), "wrong AttA must fail decryption");
    }
    ...
    fn local_chain_head_sk_advance_round_trip() {
        ...
        let pk1 = vec![0x20; 64];
        let sk1 = vec![0x22; 96];
        let device_birth_att = [0x33; 32];

        init_local_cert_chain_head_with_sk(&r, &pk0, &sk0, &device_birth_att).unwrap();
        let step1 = advance_local_cert_chain_head_with_sk(&r, &pk1, &sk1, &device_birth_att)
            .unwrap()
            .unwrap();
        assert_eq!(step1, 1);
