...
fn init_test_storage() {
    ...
    // Cache a deterministic wallet seed so the canonical signing authority can
    // re-derive the device keypair during AppRouter::new(). There is no C-DBRW
    // access gate anymore — router operations are no longer trust-gated.
