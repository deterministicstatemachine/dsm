...
| **2c-B** | Core accepted-successor prerequisite | a substrate class for the exact accepted-successor / recovery preimage | `TA_B` and `recovery_material` both reference it. It must not be the existing prost bytes canonized. **It is now also the sole blocker on complete market `Canon(B)` and the first conformant market `b`** (2c-A §5.20 field 6). |
| **2c-C** | Economic substrate closure | the namespace record, plus the transitive verification closure `TA_B` actually needs | `TA_B`'s verifier runs this walk. Assigning class numbers is *encoding* closure, not *verification* closure. |
