...
structure Evolves (w w' : World) : Prop where
  ...
  /-- A position that went to another claim never comes back (PairMutualExclusion). -/
  position_lost_stays : ∀ e, w.positionLost e = true →
    w'.positionLost e = true := by intro _ h; exact h
