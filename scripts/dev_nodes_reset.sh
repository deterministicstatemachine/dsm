...
start_nodes() {
  if [[ ! -x "dsm_storage_node/scripts/start_dev_nodes.sh" ]]; then
    die "dsm_storage_node/scripts/start_dev_nodes.sh not found or not executable."
  fi
  log "Starting 5 storage dev nodes (8080..8084)"
  (cd dsm_storage_node && ./scripts/start_dev_nodes.sh)
}
