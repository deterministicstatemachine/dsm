...
| `dsm_sdk` · SoFi unlock | SoFi Amendment S10 and storage §9 rule 11: the client keeps the completion proof of each `Final` its unlock relies on, and Core checks it against the seats. Nothing builds, keeps or checks one yet. | Open: SDK work. |
