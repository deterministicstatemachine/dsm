...
| Input | Value |
|---|---|
| Requirements | `MASTER_REQUIREMENTS.md` §8.1–§8.4: 878 canonical rows |
| Storage spec lines added after the pin | `DSM_Storage_Node_Specification.md` §14 changed in #974 (hash `8d43ac02…` → `24000775…`). Its new lines are compared as rows `STOR-014/Lnnn` (line numbers in the current file). |
| Code | `main` at `28c19cfe` |
| Crates | `dsm`, `dsm_sdk`, `dsm_storage_node`, and the shared protocol/wire code they use (`proto/dsm_app.proto`) |
| Not classified | §8.5 Open items. They are not requirements. |
| Deferred | dBTC only (owner, 2026-09-23): all of MR-DBTC-0001–0135, MR-DSM-0198, MR-DSM-0221–0237, and dBTC-specific code findings |

Code loci use the crate name and a path under its `src/`: `dsm · sofi/arith.rs · resolve` is `dsm_client/deterministic_state_machine/dsm/src/sofi/arith.rs`. Test names are the test function, qualified by module or integration-test file.
...
2. **A test that skips is not a test.** Six storage-node tests return early unless `DSM_RUN_DB_TESTS=1`; they pass on the board without running (§4.4).
...
These are the gaps that account for most of the non-Met rows. Row lists are not exhaustive; §6 is.
...
**G6. The economic-root register covers only SoFi economic positions.** Transfer, Mint and Burn advance a device root without a register write. Rows: MR-DSM-0030 (Violated), 0001, 0011, 0012.

**G7. Only unlimited-supply tokens can be created.** `token.create` refuses capped supply (`CAPPED_TOKEN_ISSUANCE_UNSUPPORTED_IN_BETA`), `economic/issuance.rs::check_issuance_permitted` refuses `FiniteSupplyCap`, creation forces `initial_alloc = 0`, and supply arrives only through repeated discretionary `token.mint`. `check_market_leg_permitted` has no callers. Rows: MR-SOFI-0306, 0307 (Violated), 0308, 0309, 0311, 0319–0321, 0323–0325.
...
**G9. The per-device SMT evicts relationship heads.** `SparseMerkleTree::update_leaf` evicts the oldest leaf once more than `max_leaves` (1024 at genesis) are held; the evicted head then reads as absent. Rows: MR-DSM-0116, 0121 (Violated).
...
**G11. No cross-receipt consumption registry for DLV generations.** `vault/dlv_manager.rs` documents that a double claim across stitched receipts is not enforced. Rows: MR-DSM-0142, 0152, 0164, 0166, 0168, 0170, 0174, 0176, 0188.

## 4 CODE → SPEC

What the backend does that no requirement asks for, or that a requirement forbids. §3 G4 covers the node's legacy store; it is not repeated here.

### 4.1 Stand-ins on production paths
...
### 4.2 Stand-ins with no production caller
...
### 4.3 Code with no production caller
...
### 4.4 Tests that report green without testing the shipped path
...
|---|---|---|---|---|---|---|---|
| DSM high-level (MR-DSM) | 270 | 65 | 121 | 27 | 10 | 29 | 18 |
| SoFi (MR-SOFI) | 329 | 218 | 70 | 19 | 5 | 17 | 0 |
| dBTC (MR-DBTC) | 135 | 0 | 0 | 0 | 0 | 0 | 135 |
| Storage node (MR-STOR) | 144 | 30 | 35 | 48 | 14 | 17 | 0 |
| Storage §14 lines added after the pin (STOR-014) | 11 | 9 | 1 | 1 | 0 | 0 | 0 |
| **All** | **889** | **322** | **227** | **95** | **29** | **63** | **153** |

## 6 Per-requirement results

### 6.1 DSM high-level explainer

| MR-ID | Status | Code (crate · file · function) | Test | Gap / note |
|---|---|---|---|---|
| MR-DSM-0001 | Partial | dsm · types/device_state.rs · `advance` (balance subtraction only); economic/register.rs · `economic_root_register_key` (SoFi economic position only) | economic_lineage_register::each_position_of_each_identity_is_its_own_cell | Non-equivocation register covers only SoFi `economic_position`; ordinary Transfer/Mint/Burn rely only on balance subtraction, no register. |
...
| MR-DSM-0017 | Partial | dsm · sofi/arith.rs::CellResolution (43-51) | sofi::arith::tests::leader_held_settles_the_race_before_finality | SoFi-cell only; general Pending/Unavailable not traced. |
...
| MR-DSM-0030 | Violated | dsm · types/device_state.rs · `advance` (Transfer/Mint/Burn deltas applied, no register write) | none | Value-moving advances outside SoFi are never registered in the economic-root register. |
...
| MR-DSM-0041 | Partial | ordering not located on the production path | no test found | The earlier locus (transition.rs) has no production caller. |
| MR-DSM-0042 | Partial | ordering not located on the production path | no test found | The earlier locus (transition.rs) has no production caller. |
...
### 6.2 SoFi settlement specification
...
| MR-SOFI-0029 | Met | dsm · sofi/conformance.rs::`Validation::and/all` | `sofi/conformance.rs::invalid_dominates_and_unavailable_never_becomes_invalid` | — |
| MR-SOFI-0030 | Met | dsm · sofi/validation.rs::`validate` | `sofi/validation.rs::missing_evidence_is_unavailable_never_invalid` | — |
...
| MR-SOFI-0068 | Partial | dsm · economic/register.rs::`RootRegisterProfile::verify_candidate`:126-161; ccb/mod.rs::`storage_set_id`:830 | no test found | No test found. |
...
| MR-SOFI-0097 | Met | dsm · sofi/storage.rs::`keep_verifying`/`keep_all_verifying` | `sofi/storage.rs::over_budget_is_unavailable_never_none` | — |
...
| MR-SOFI-0204 | Met | dsm · sofi/validation.rs::validate (RouteValidation); sofi/conformance.rs::fulfillment_conformance (independent predicates) | sofi/conformance.rs::tests::invalid_dominates_and_unavailable_never_becomes_invalid | — |
...
| MR-SOFI-0272 | Met | dsm_sdk · sdk/sofi_sdk.rs · `draft` | `unavailable_stops_the_producer_and_nothing_is_produced` | — |
...
### 6.4 Storage node specification
...
| MR-STOR-0030 | Met | dsm_storage_node · api/cells.rs `put_cell`/`put_cells` | cells_keep_everything.rs `a_second_value_at_a_key_is_kept_after_the_first_never_refused`, `only_malformed_requests_are_refused` | — |
...
| MR-STOR-0045 | Partial | — | not traced | Recognition-before-read not verified across SDK read sites |
...
| MR-STOR-0055 | Met | dsm_sdk · sdk/storage_set.rs `compute_storage_set_id`, `StorageSet::new` | `set_id_is_order_independent_and_length_prefixed` | — |
...
| MR-STOR-0109 | Met | dsm_storage_node · api/vault/paidk.rs · `require_paidk`; callers api/objects/store.rs::`put_object`/`delete_object_proto`, api/transport/b0x.rs::`submit_b0x_envelope` | tests/paidk_gating.rs::`t_a_unpaid_device_rejected` | Wired, but only on the legacy object-store + b0x paths — the current Part II keyed-cell write (`api/cells.rs::put_cell`) carries no PaidK gate at all (see note under 0117). |
...
| MR-STOR-0122 | Not code | — | — | Proof obligations. |
| MR-STOR-0123 | Not code | — | — | Proof obligations. |
| MR-STOR-0124 | Not code | — | — | Proof obligations. |
| MR-STOR-0125 | Not code | — | — | Proof obligations. |
| MR-STOR-0126 | Not code | — | — | Proof obligations. |
| MR-STOR-0127 | Not code | — | — | Proof obligations. |
...
| MR-STOR-0143 | Not code | — | — | Proof obligation. |
| MR-STOR-0144 | Not code | — | — | Proof obligation. |

### 6.5 Storage §14 lines added after the pin
